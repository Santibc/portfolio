...
@section('content')
        ...
        <div class="row align-items-center">
          <div class="col-lg-6">
            <div class="hero-content">
              ...
              <h2 class="mb-4"><span>{{ $heroSection->subtitle ?? 'Top Quality Guaranteed' }}</span></h2>
          ...
          <div class="col-lg-6">
            <div class="hero-image">
              <img src="{{ $heroSection && $heroSection->hero_image_path ? asset($heroSection->hero_image_path) : asset('images/mujer.png') }}" class="img-fluid floating" alt="Guillen Cleaning Services">
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="swiper init-swiper">
          ...
          <div class="swiper-wrapper align-items-center">
            <div class="swiper-slide">
              <a href="{{ $socialMedia->facebook_url ?? 'https://www.facebook.com' }}" target="_blank" rel="noopener noreferrer">
                <img src="{{ asset('images/facebook.png') }}" class="img-fluid" alt="Facebook">
              </a>
            </div>
            <div class="swiper-slide">
              <a href="{{ $socialMedia->instagram_url ?? 'https://www.instagram.com' }}" target="_blank" rel="noopener noreferrer">
        ...
        <div class="row align-items-center">
          ...
          <div class="col-lg-6">
            <div class="about-image">
              <img src="{{ $homeAbout && $homeAbout->image_path ? asset($homeAbout->image_path) : asset('images/paginaanterior/imagenluegodeltitulo.avif') }}" alt="We Are Guillen Cleaning" class="img-fluid">
            </div>
          </div>
          ...
          <div class="col-lg-6">
            <div class="content">
              <h2>{{ $homeAbout->title ?? 'WE ARE GUILLEN CLEANING LLC' }}</h2>
              <p class="lead">{{ $homeAbout->lead_text ?? 'Excellence and professionalism are first when it comes to our Residential and Commercial Cleaning Services.' }}</p>

              <p>{{ $homeAbout->description ?? 'We are constantly improving our services, staying up-to-date on all the latest industry advancements, and bringing our knowledge to your doorstep. Since 2009, our goal has remained the same—to provide reliable services and make sure our clients know we are professionals they can trust.' }}</p>

              <!-- Stats Row -->
              <div class="stats-row">
                <div class="stat-item">
                  <h3><span data-purecounter-start="0" data-purecounter-end="{{ $homeAbout->years_experience ?? 16 }}" data-purecounter-duration="1" class="purecounter"></span>+</h3>
                  <p>Years Experience</p>
                </div>
                <div class="stat-item">
                  <h3><span data-purecounter-start="0" data-purecounter-end="{{ $homeAbout->happy_clients ?? 500 }}" data-purecounter-duration="1" class="purecounter"></span>+</h3>
                  <p>Happy Clients</p>
                </div>
                <div class="stat-item">
                  <h3><span data-purecounter-start="0" data-purecounter-end="{{ $homeAbout->client_satisfaction ?? 100 }}" data-purecounter-duration="1" class="purecounter"></span>%</h3>
              ...
              <div class="cta-wrapper">
                <a href="{{ $homeAbout->cta_button_url ?? route('nosotros') }}" class="btn-cta">
                  <span>{{ $homeAbout->cta_button_text ?? 'Learn More About Us' }}</span>
    ...
    <section id="services" class="services section">
      ...
      <div class="container section-title">
        <h2>{{ $homeServices->section_title ?? 'Our Services' }}</h2>
        <p>{{ $homeServices->section_description ?? 'Professional cleaning solutions for your residential and commercial needs' }}</p>
      </div><!-- End Section Title -->

      <div class="container">

        <h3 class="text-center mb-5">{{ $homeServices->commercial_title ?? 'Commercial Cleaning Services' }}</h3>

        <!-- Commercial Images Gallery -->


        <div class="row gy-4 mb-5">
          @if($serviceItems->where('type', 'commercial')->count() > 0)
            @foreach($serviceItems->where('type', 'commercial') as $service)
        ...
        <div class="row gy-4 mb-5">
          @if($serviceImages->where('type', 'commercial')->count() > 0)
            @foreach($serviceImages->where('type', 'commercial') as $image)
              <div class="col-lg-6 col-md-6 text-center">
                <div class="service-image-wrapper">
                  <img src="{{ asset($image->image_path) }}" alt="{{ $image->alt_text ?? 'Commercial Cleaning Services' }}" class="img-fluid rounded shadow">
                </div>
              </div>
            @endforeach
          @else
        ...
        <h3 class="text-center mb-5">{{ $homeServices->residential_title ?? 'Residential Cleaning Services' }}</h3>

        <!-- Residential Images Gallery -->


        <div class="row gy-4 mb-5">
          @if($serviceItems->where('type', 'residential')->count() > 0)
            @foreach($serviceItems->where('type', 'residential') as $service)
              <div class="col-lg-6 col-md-6">
                <div class="service-card">
                  <div class="service-icon">
                    <i class="{{ $service->icon_class }}"></i>
        ...
        <div class="row gy-4 mb-5">
          @if($serviceImages->where('type', 'residential')->count() > 0)
            @foreach($serviceImages->where('type', 'residential') as $image)
              <div class="col-lg-6 col-md-6 text-center">
                <div class="service-image-wrapper">
                  <img src="{{ asset($image->image_path) }}" alt="{{ $image->alt_text ?? 'Residential Cleaning Services' }}" class="img-fluid rounded shadow">
                </div>
              </div>
            @endforeach
          @else
        ...
        <div class="row mt-5">
          <div class="col-12">
            <div class="alert alert-info text-center">
              <p class="mb-0">{{ $homeServices->eco_friendly_note ?? '<strong>Eco-Friendly Commitment:</strong> In both our commercial and residential cleaning services, we use eco-friendly cleaning products, employ highly trained and professional staff, and tailor our services to meet your specific needs. Our goal is to provide a clean, healthy, and welcoming environment for your home or business.' }}</p>
            </div>
          </div>
        </div>
    ...
    <section id="contact" class="contact section">
      ...
      <!-- Section Title -->
      <div class="container section-title">
        <h2>{{ $contactInfo->contact_hero_title ?? 'Get Your Free Estimate' }}</h2>
