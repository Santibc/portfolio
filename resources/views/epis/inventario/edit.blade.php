...
<div class="container-fluid py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h1 class="h3 mb-1">Editar EPI</h1>
            <p class="text-muted mb-0">{{ $epiInventario->catalogo->nombre ?? 'EPI' }} {{ $epiInventario->numero_serie ? '- ' . $epiInventario->numero_serie : '' }}</p>
        </div>
        <a href="{{ route('epi-inventario.show', $epiInventario) }}" class="btn btn-outline-secondary">
            <i class="bi bi-arrow-left me-2"></i>Volver
        </a>
    </div>

    <form action="{{ route('epi-inventario.update', $epiInventario) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="row g-4">
            <!-- Columna Principal -->
                                ...
                                <select name="epi_catalogo_id" class="form-select @error('epi_catalogo_id') is-invalid @enderror" required id="epi_catalogo_id">
                                    ...
                                    @foreach($catalogos as $catalogo)
                                        ...
                                        <option value="{{ $catalogo->id }}"
                                                ...
                                                {{ old('epi_catalogo_id', $epiInventario->epi_catalogo_id) == $catalogo->id ? 'selected' : '' }}>
                                            {{ $catalogo->nombre }}
                                        </option>
                                    @endforeach
                                    @if($currentCategoria !== null)
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <input type="text" name="numero_serie" class="form-control @error('numero_serie') is-invalid @enderror"
                                       value="{{ old('numero_serie', $epiInventario->numero_serie) }}" placeholder="Ej: SN-2025-001">
                                @error('numero_serie')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <input type="date" name="fecha_compra" class="form-control @error('fecha_compra') is-invalid @enderror"
                                       value="{{ old('fecha_compra', $epiInventario->fecha_compra?->format('Y-m-d')) }}">
                                @error('fecha_compra')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6" id="fecha_caducidad_container">
                                ...
                                <input type="date" name="fecha_caducidad" class="form-control @error('fecha_caducidad') is-invalid @enderror"
                                       value="{{ old('fecha_caducidad', $epiInventario->fecha_caducidad?->format('Y-m-d')) }}">
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="input-group">
                                    <input type="number" name="coste" step="0.01" min="0"
                                           ...
                                           value="{{ old('coste', $epiInventario->coste) }}" placeholder="0.00">
            ...
            <div class="col-lg-4">
                ...
                <div class="card border-0 shadow-sm mb-4">
                    ...
                    <div class="card-body">
                        ...
                        @endphp
                        <span class="badge bg-{{ $estadoColors[$epiInventario->estado] ?? 'secondary' }} fs-6">
                            {{ ucfirst(str_replace('_', ' ', $epiInventario->estado)) }}
                        </span>
                        <p class="text-muted small mt-2 mb-0">
                            El estado no se puede cambiar directamente. Use las acciones de entrega, devolucion o baja.
                        </p>
                    </div>
                </div>
                ...
                <div class="card border-0 shadow-sm">
                    ...
                    <div class="card-body">
                        <textarea name="notas" class="form-control" rows="4"
                                  placeholder="Observaciones adicionales...">{{ old('notas', $epiInventario->notas) }}</textarea>
                    </div>
                </div>
            </div>
        </div>
