        ...
        <div class="category-controls mb-4">
            <div class="row align-items-center">
                <div class="col-12 col-md-6">
                    <h1 class="category-title">{{ $categoriaSeleccionada ? $categoriaSeleccionada->nombre : 'Productos' }}
                    </h1>
                    <p class="category-count">Mostrando <span class="js-products-count">{{ $productos->total() }}</span>
                        productos</p>
                </div>
                <div class="col-12 col-md-6 text-md-end">
                    <div class="d-flex justify-content-md-end gap-2 align-items-center">
                        <!-- Mobile Filter Button -->
                        <button class="btn btn-outline-secondary d-md-none" data-bs-toggle="modal"
                            data-bs-target="#modal-filters">
                            <svg class="icon-inline mr-1">
                        ...
                        <form method="GET" action="{{ route('tienda.categorias', $empresa->slug) }}" id="sortForm"
                            class="d-none d-md-inline-block mb-0">
                            ...
                            <div class="form-group mb-0">
                                ...
                                <select name="orden" class="form-select form-select-sm" aria-label="Ordenar por"
                                    ...
                                    <option value="nombre" {{ request('orden') == 'nombre' ? 'selected' : '' }}>Nombre
                                    </option>
                                </select>
                            </div>
                        </form>
            ...
            <aside class="category-sidebar d-none d-md-block">
                            ...
                            <div class="row g-2 mb-3">
                                <div class="col-6">
                                    <input type="number" name="precio_min" class="form-control form-control-sm"
                                        placeholder="Mínimo" id="priceMin" value="{{ request('precio_min', $precioMin) }}"
                                        min="{{ $precioMin }}" max="{{ $precioMax }}">
                                </div>
                                <div class="col-6">
                                    <input type="number" name="precio_max" class="form-control form-control-sm"
                                        placeholder="Máximo" id="priceMax" value="{{ request('precio_max', $precioMax) }}"
                                        min="{{ $precioMin }}" max="{{ $precioMax }}">
                                </div>
                            </div>
                ...
                @if (request()->anyFilled(['categoria', 'precio_min', 'precio_max', 'buscar']))
                    <div class="filter-clear">
                        <a href="{{ route('tienda.categorias', $empresa->slug) }}"
                            class="btn btn-link text-decoration-none">Limpiar filtros</a>
                    </div>
                @endif
            </aside>
            ...
            <div class="category-products">
                <div class="products-grid">
                    @forelse($productos as $index => $producto)
                        @php
                            $precioActual = is_object($producto->precio_actual)
                                ? $producto->precio_actual->precio
                                : $producto->precio_actual;
                            $precioAnterior =
                                is_object($producto->precio_actual) && isset($producto->precio_actual->precio_anterior)
                        ...
                        <div class="product-card-cat">
                            ...
                            <div class="product-info">
                                <h3 class="product-name">
                                    <a
                                        href="{{ route('tienda.producto', [$empresa->slug, $producto->id]) }}">{{ $producto->nombre }}</a>
                                </h3>
                                <div class="product-price-container">
                                    @if ($descuentoActivo)
                                        <span
                                            class="product-price-old">${{ number_format($precioActual, 0, ',', '.') }}</span>
                                        <span
                                            class="product-price">${{ number_format($precioConDescuento, 0, ',', '.') }}</span>
                                    @else
                                        <span
                                            class="product-price">${{ number_format($precioActual, 0, ',', '.') }}</span>
                                    @endif
                                </div>
                                <p class="product-installments" style="font-size: 11px; color: #6c757d; margin-top: 4px;">
                                    3 x
                                    ${{ number_format(($descuentoActivo ? $precioConDescuento : $precioActual) / 3, 0, ',', '.') }}
                                    sin interés</p>
                            </div>
                        </div>
                    @empty
        ...
        <div class="modal-dialog modal-dialog-scrollable modal-fullscreen-md-down">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="filter-section mb-4">
                        <h6 class="filter-subtitle">Precio</h6>
                        <form id="priceRangeFormMobile" method="GET"
                            action="{{ route('tienda.categorias', $empresa->slug) }}">
                            @foreach (request()->except(['precio_min', 'precio_max']) as $key => $value)
                                @if ($value)
                                    <input type="hidden" name="{{ $key }}" value="{{ $value }}">
                                @endif
                            ...
                            <div class="price-range-container">
                                <div class="row g-2 mb-3">
                                    <div class="col-6">
                                        <input type="number" name="precio_min" class="form-control form-control-sm"
                                            placeholder="Mínimo" value="{{ request('precio_min', $precioMin) }}"
                                            min="{{ $precioMin }}" max="{{ $precioMax }}">
                                    </div>
                                    <div class="col-6">
                                        <input type="number" name="precio_max" class="form-control form-control-sm"
                                            placeholder="Máximo" value="{{ request('precio_max', $precioMax) }}"
                                            min="{{ $precioMin }}" max="{{ $precioMax }}">
                                    </div>
                                </div>
                                <button type="submit" class="btn btn-sm btn-outline-secondary w-100">Aplicar</button>
            ...
            <div class="modal-content">
                    ...
                    <form method="GET" action="{{ route('tienda.categorias', $empresa->slug) }}" id="sortFormMobile">
                        ...
                        <div class="sort-options">
                            <label class="sort-option">
                                <input type="radio" name="orden" value=""
                                    {{ !request('orden') ? 'checked' : '' }}>
                            ...
                            <label class="sort-option">
                                <input type="radio" name="orden" value="nombre"
                                    {{ request('orden') == 'nombre' ? 'checked' : '' }}>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-primary"
                        onclick="document.getElementById('sortFormMobile').submit()">Aplicar</button>
                </div>
            </div>
        </div>
    </div>
