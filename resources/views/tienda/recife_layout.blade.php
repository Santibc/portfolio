...
<form class="js-search-form search-form " id="recifeSearchForm" method="get">
    <div class="form-group m-0">
        <input class="js-search-input form-control search-input background-secondary" autocomplete="off" type="search" name="q" placeholder="Buscar" aria-label="Buscar" id="recifeSearchInput">
...
DOMContentLoaded.addEventOrExecute(() => {
    ...
    const recifeSearchForm = document.getElementById('recifeSearchForm');
    const recifeSearchInput = document.getElementById('recifeSearchInput');

    if (recifeSearchForm && recifeSearchInput) {
        recifeSearchForm.addEventListener('submit', function(e) {
            e.preventDefault();
            const query = recifeSearchInput.value.trim();
            if (query) {
                window.location.href = "{{ route('tienda.categorias', $empresa->slug) }}?buscar=" + encodeURIComponent(query);
            }
        });

        recifeSearchInput.addEventListener('keypress', function(e) {
            if (e.key === 'Enter') {
                e.preventDefault();
                recifeSearchForm.dispatchEvent(new Event('submit'));
            }
        });
    }
