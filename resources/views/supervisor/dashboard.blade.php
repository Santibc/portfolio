<x-app-layout>
    <x-agromarket.page-header
        ...
        subtitle="Panel de control y estadísticas generales (Solo lectura)"
    />

    <div class="alert alert-info mb-4">
        <i class="fas fa-info-circle"></i>
        Este dashboard es de solo lectura. No puede realizar acciones de aprobación o modificación.
    </div>
    ...
    {{-- Solicitudes Pendientes (Solo vista) --}}
    <div class="dashboard-row mt-4">
        {{-- Retiros Pendientes --}}
                    ...
                    @if($solicitudes_pendientes['retiros']['cantidad'] > 0)
                        ...
                        @if($solicitudes_pendientes['retiros']['cantidad'] > 5)
                            <div class="text-muted text-center mt-3 small">
                                + {{ $solicitudes_pendientes['retiros']['cantidad'] - 5 }} más
                            </div>
                        @endif
                    @else
                    ...
                    @if($solicitudes_pendientes['kyc']['cantidad'] > 0)
                        <div class="pending-list">
                            @foreach($solicitudes_pendientes['kyc']['items'] as $kyc)
                                <div class="pending-item">
                                    <div class="pending-info">
                                        <strong>{{ $kyc->usuario->name }}</strong>
                        ...
                        @if($solicitudes_pendientes['kyc']['cantidad'] > 5)
                            <div class="text-muted text-center mt-3 small">
                                + {{ $solicitudes_pendientes['kyc']['cantidad'] - 5 }} más
                            </div>
                        @endif
                    @else
                ...
                <div class="card-body">
                    @if($solicitudes_pendientes['proyectos']['cantidad'] > 0)
                        ...
                        @if($solicitudes_pendientes['proyectos']['cantidad'] > 5)
                            <div class="text-muted text-center mt-3 small">
                                + {{ $solicitudes_pendientes['proyectos']['cantidad'] - 5 }} más
                            </div>
                        @endif
                    @else
