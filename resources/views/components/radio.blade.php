...
<label for="{{ $id }}" class="flex items-start gap-2.5 cursor-pointer select-none">
    <input
        ...
        @checked($checked)
        {{ $attributes->merge([
            'class' => 'mt-0.5 h-4 w-4 border-cream-300 text-primary-500 focus:ring-2 focus:ring-primary-500/40 focus:ring-offset-0 dark:border-cream-600 dark:bg-cream-900',
        ]) }}
    />
