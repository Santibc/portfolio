...
{{-- Import Modal --}}
@can('editar_catalogo_items')
<x-sinden.modal id="importModal" title="Importar Items desde Excel" size="lg">
    <div class="mb-3">
        <p class="text-muted mb-2">Suba un archivo Excel (.xlsx) con los items a importar. Los items con codigo existente seran actualizados; los nuevos seran creados.</p>
        <a href="{{ route('recepcion.items.import-template') }}" class="btn btn-sm btn-outline-secondary">
...
function ejecutarImportacion() {
    ...
    fetch('{{ route("recepcion.items.import-excel") }}', {
        method: 'POST',
        headers: {
...
function cargarHistorialImport() {
    ...
    fetch('{{ route("recepcion.items.import-history") }}', {
...
function verDetalleImport(importId) {
    ...
    fetch('{{ url("recepcion/items/import-detail") }}/' + importId, {
        headers: { 'Accept': 'application/json', 'X-Requested-With': 'XMLHttpRequest' }
    })
    .then(function(r) { return r.json(); })
    .then(function(data) {
        ...
        html += '<thead class="table-light"><tr><th width="60">Fila</th><th width="120">Codigo</th><th width="120">Accion</th><th>Mensaje</th></tr></thead><tbody>';
